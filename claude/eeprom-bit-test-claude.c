...
void initEEPROM(EEPROM* eeprom) {
	...
	// FIX: maxHardFaultBits was declared in the header but never set anywhere,
	// so it always read back as 0. Set it here so it actually reflects the
	// theoretical ceiling (every bit in the device stuck), in case you want
	// to use it later as a sanity bound or an exit condition.
	...
	// FIX: added NULL checks for all three allocations. Previously an
	// allocation failure here would go unnoticed until the first NULL
	// dereference deep inside logger(), crashing without a clear message.
	// eepromClose() safely frees whichever of these three succeeded
	// (each free() inside it is already NULL-guarded) before exiting.
...
// ADDED: restoreAndVerifyByte() is a new helper function that replaces the
// old per-bit-index restore/verify block that lived directly inside logger()'s
// bit loop. That old placement caused two bugs:
//   1. it issued up to 8 redundant restore writes for a single mismatched byte
//      (one per bit index once bitRequiresRestore latched true)
//   2. the verify/hard-fault-check branch could only run on bits that had NOT
//      flipped (because it only executed while bitRequiresRestore was still
//      false), so hardFault could never actually be set to true.
//
...
bool restoreAndVerifyByte(EEPROM* eeprom, int reg, uint8_t mismatchMask, time_t elapsedTime) {
	...
	for (int attempt = 0; attempt < RESTORE_VERIFY_RETRIES; attempt++) {
		...
		// FIX: this delay was missing from the restore-write path entirely
		// in the previous version (it only appeared in the old, unreachable
		// verify branch). The EEPROM needs its full write-cycle time before
		// a read will reliably reflect the new value.
		delay(WRITE_DELAY);
		...
		if ((uint8_t)verifyData == eeprom->priorState[reg]) {
			// restore verified successfully -- nothing further to do.
			// priorState[reg] is intentionally left unchanged: it already
			// holds the correct reference value, and we never assign the
			// raw read-back value into it (that was the root cause of the
			// oscillation bug from the earlier version of this file).
			return true;
		}
	...
	return false; // exhausted retries without a clean verification
}
...
void logger(time_t startTime, FILE* csv_file, EEPROM* eeprom) {
	...
	/*
	updated pseudocode (reflects the restructuring below):
	...
	*/
	...
	for (int reg = 0; reg < eeprom->size; reg++) {
		...
		// successful read, data >= 0 -- NOTE: this correctly includes data == 0,
		// which is a valid byte value (all bits flipped to 0) and must not be
			...
			for (uint8_t i = 0; i < 8; i++) {
				...
				if (currBit != priorBit) { // found a bit that flipped
					...
					if (lookupIndex == -1) {
						...
						currentBitRecord->timesFlipped  = 0;			// ADDED
						currentBitRecord->firstFlipTime = elapsedTime;	// ADDED
						currentBitRecord->lastFlipTime  = elapsedTime;	// ADDED
					} else {
						currentBitRecord = &eeprom->flippedBitsPtr[lookupIndex];
					}
					...
					if (currentBitRecord->hardFault == false) {
						currentBitRecord->timesFlipped++;			// ADDED
						currentBitRecord->lastFlipTime = elapsedTime;	// ADDED
						if (priorBit == 1 && currBit == 0) eeprom->oneToZeroFlips += 1;
						if (priorBit == 0 && currBit == 1) eeprom->zeroToOneFlips += 1;
						eeprom->numBitFlips += 1; // ADDED: this was never actually incremented
												  // anywhere in the previous version, even though
												  // it's used throughout for capacity checks/logging
					}
				}
			}
		...
		// FIX: %zu used consistently for size_t fields instead of %ld (technically
		// undefined behavior when the format specifier's signedness doesn't
		// match the argument's, even though it often "works" on 64-bit Linux)

		// monitor test
		printf("\rTime: %-5d byte: %-6d data: %-3d, Bit Flips: %-5zu, 1->0: %-5zu, 0->1: %-5zu, Hard Faults: %-5zu, Read Fails: %-5zu      ",
...
int main(void) {
	...
	initEEPROM(&eeprom);	// note: this function initializes an eeprom by setting all of its bits to a #defined VALUE
							// once this value is set the connection is kept open and reused for the rest of
							// the run -- logger() no longer reopens/closes the i2c connection on every call.
							// FIX: previously wiringPiI2CSetup() was called at the top of every logger()
							// invocation, which added unnecessary syscall overhead every pass and gave no
							// real recovery benefit (it doesn't reset the bus or the chip, just reopens a
							// file descriptor to the same address). The connection is now opened once here
							// and only closed/reopened deliberately as part of failure-recovery handling,
							// were you to add that.
	...
	// FIX: added "Read Failures" column to match the new eeprom->readFailures field
	...
	free(eeprom.bitLookup); // FIX: bitLookup was allocated but never freed at program exit
